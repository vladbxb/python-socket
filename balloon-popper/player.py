...
class Score(arcade.Text):
    ...
    def increase(self) -> None:
        ...
        self._score = increase_score_num(self._score)

    def decrease(self) -> None:
        """Decreases the score number. The player is penalized more for popping wrong balloons."""
        self._score = max(decrease_score_num(self._score), 0)
...
class BalloonManager:
    ...
    def pop_top(self, position: tuple[float, float]) -> bool:
        ...
        x, y = position
        balloons_clicked: list[arcade.sprite.Sprite] = arcade.get_sprites_at_point((x, y), self._balloons)
...
class Player:
    ...
    def __init__(self, player_color: str):
        ...
        self._score_position = SCORE_POSITIONS[player_color]
        self._score: Score = Score(*self._score_position, SCORE_COLORS[player_color])
